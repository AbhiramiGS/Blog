...
export function ExpandableCardDemo({ cards }: ExpandableCardDemoProps) {
  ...
  return (
    <>
      ...
      <ul className="columns-3 mx-auto space-y-4 gap-4 p-20">
        {cards.map((card) => (
          <motion.div
            layoutId={`card-${card.title}-${id}`}
            key={card.id}
            onClick={() => setActive(card)}
            className="p-4 break-inside-avoid hover:bg-neutral-300 dark:hover:bg-neutral-800 rounded-xl cursor-pointer"
          >
